...
!!! note
    Some configuration settings, such as the settings for the board and tagging features, were moved from the profiles-config.xml file to the profiles-policy.xml file. For more information about the settings that you can configure in the `profiles-policy.xml` file, see [Profile-types](../customize/r_admin_profiles_ovr_types.md).
...
organizationalStructureEnabled.enabled
...
: `platformCommandBatchSize`: Specifies the maximum number of events attempted to process in each event run.

...
|
|CLFWX0010E: The JMS QueueConnectionFactory or Queue for CommunityInbound could not be looked up. Check that the QCF and Queue are defined in WebSphere and bound to the application. Check the nested exception for more details.|There was an error while using the JMS infrastructure in the News repository.|1.  Verify that the News repository is installed correctly.
2.  When using the advanced stand-alone install, verify that the JMS configuration has been set up correctly.
...
|CLFWX0151E: Could not obtain default email preferences from notification-config.xml. Check nested exception for more details.|There was an error when retrieving the default email digest settings from notification-config.xml.|1.  Verify that notification-config.xml exists under LotusConnections-Config.xml.

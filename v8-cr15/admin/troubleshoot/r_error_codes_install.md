...
|CLFRP0010E: No application server node detected under Deployment Manager, HCL Connections needs at least 1 node to proceed!|The installation wizard requires at least one node under the Deployment Manager.|Make sure that a node under the Deployment Manager is available and install again.|
...
|CLFRP0013E: Please click "Validate" button after all inputs were filled in.|After providing input data for the installation, you must request that the installation wizard validate the data.|Click "Validate" to confirm your inputs.|

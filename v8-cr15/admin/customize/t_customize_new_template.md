...
-   To customize **profile photo types**:
    ...
    -   Profile photo type
        ...
        -   `attach` Default value, includes the profile photo as an email attachment when the email is generated. This option is best for mail clients that cannot load authenticated profile photo URLs, but attachments increase email size and can affect performance.
        -   `url` Displays the photo by embedding the HCL Connections profile URL in the email.
        -   `stock` Displays the application icon instead of the profile image.

    **Note:** The `photoType` property is used only when `globalIncludeProfileImageInEmail` is set to `true` in the `notification-config.xml` file. Otherwise, enhanced email notifications display the stock application image.
    -   Image background color

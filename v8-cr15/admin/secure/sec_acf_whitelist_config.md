# Configure active content filters (allowlists) {#sec_acf_whitelist_config}
...
2.  Modify the `acp-configkey__default.xml` file to reference the new allowlist XML file (`ojhs-whitelist-update_description.xml`).
...
3.  Modify the `LotusConnectionsConfig.xml` file so that its `sloc` entry `acf_config_file` points to the new `acp-configkey__update_description.xml` file.
    ...
    TTo edit configuration files, you must use the `wsadmin` client. See [Starting the wsadmin client](../admin/t_admin_wsadmin_starting.md) for details.
    ...
    1.  Enter the following command to load the HCL Connections configuration file: 
        ...
        If you are prompted to specify a service to connect to, type `1` to select the first node in the list. Most commands can be run on any node. However, if a command reads from or writes to a file using a local file path, you must select the node where the file is stored. This node selection is not used by the `wsadmin` client when you are making configuration changes.
    ...
    5.  Add the `acp-configkey__default-update_description.xml` file name to the `acf_config_file=` element as shown in the following example.
        ...
        <sloc:serviceReference 
         ...
         serviceName="blogs" 
         ssl_enabled="true" 
         **acf_config_file="acp-configkey__pilot.xml">**
          <sloc:href>

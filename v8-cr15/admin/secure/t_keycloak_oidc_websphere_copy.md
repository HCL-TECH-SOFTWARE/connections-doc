...
Single sign-on is accomplished by setting up a trust relationship between the Connections server and Keycloak using the WebSphere OpenID Connect Relying Party Trust Association Interceptor (OIDC Relying Party TAI). This requires installing the WebSphereOIDCRP application on each cluster and configuring the TAI.

## Before you begin

Complete the steps in [Configuring Keycloak as an OIDC provider for Connections](t_keycloak_config_conn_oidc.md), which will provide the values that you will need in the following procedure.

!!! important

    The OIDC configuration below uses a medium topology deployment as its primary example, which typically involves four clusters and, consequently, four OIDC providers.

    The number of OIDC providers required for your environment is directly dependent on your deployment type (small, medium, or large), with the rule that each cluster must have its own provider.

    If your environment is a small or large topology, you must adjust the number of providers based on your specific cluster count. 
    
    The examples below also show both `excludedPathFilter` and `interceptedPathFilter`: typically, `excludedPathFilter` contains global URL patterns that should not be intercepted and may be shared across providers, while `interceptedPathFilter` should reflect the application endpoints hosted on the cluster associated with each provider and therefore can differ between providers.

    As a general guideline, review which applications and context roots are deployed on each cluster and configure each provider's `interceptedPathFilter` to protect only the intended application URLs. After updating filters, synchronize nodes and validate authentication behavior in a test environment to confirm only the desired endpoints require OIDC authentication.
...
1.  For every cluster, deploy WebSphereOIDCRP. To install the WebSphereOIDCRP application, follow the instructions in the WebSphere documentation [Configuring an OpenID Connect Relying Party](https://www.ibm.com/docs/en/was-nd/8.5.5?topic=users-configuring-openid-connect-relying-party) and follow *Step 1* in *"Install the OpenID Connect application"*. <p>In this example, the Connections applications are installed across the four clusters, AppsCluster, InfraCluster, PushCluster, and UtilCluster.</p>
    ...
    !!! note 
        ...
        The OIDC Relying Party TAI code is shipped as part of WebSphere Application Server. For best results, ensure your WebSphere Fix Pack is up to date to guarantee you are using the most current version of the TAI (Trust Association Interceptor). Upgrade to the latest Websphere Fix Pack and check if your environment has the latest version from  [Obtaining WebSphere OpenID Connect (OIDC) latest version](https://www.ibm.com/support/pages/node/290565). 
...
5.  Configure Global Security Custom Properties by clicking **Security** \> **Global security** \> **Custom Properties** and setting these properties to the following values:

    |Property|Value|
    |--------|-----|
    |com.ibm.websphere.security.disableGetTokenFromMBean<br>**Note:** If this property does not exist, add it.     |false|
    |com.ibm.websphere.security.DeferTAItoSSO | **Delete this property if it exists**<br>**Note:** Delete this property if it exists. This property was previously used in the default configuration of all installed servers, but it is now strictly utilized as part of SAML configurations. Unless your environment specifically uses SAML single sign-on, this property must be removed. If you are using SAML, ensure its value is explicitly set to `com.ibm.ws.security.web.saml.ACSTrustAssociationInterceptor` (multiple comma-separated values are not supported).|
...
10.  Add the following properties and values.
    ...
    !!! note 
        ...
        Use the values for `{client_id}`, `{client_secret}`, and `{realm}` from the previous procedure.
    ...
    |provider_1.useRealm | {realm} Use the realm value from Keycloak configuration |
    |provider_2.useRealm | {realm} Use the realm value from Keycloak configuration |
    |provider_3.useRealm | {realm} Use the realm value from Keycloak configuration |
    |provider_4.useRealm | {realm} Use the realm value from Keycloak configuration |
    ...
    |provider_1.discoveryEndpointUrl | https://{keycloak_server}/realms/{realm}/.well-known/openid-configuration |
    |provider_2.discoveryEndpointUrl | https://{keycloak_server}/realms/{realm}/.well-known/openid-configuration |
    |provider_3.discoveryEndpointUrl | https://{keycloak_server}/realms/{realm}/.well-known/openid-configuration |
    |provider_4.discoveryEndpointUrl | https://{keycloak_server}/realms/{realm}/.well-known/openid-configuration |
    |provider_1.endSessionEndpointEnabled | true |
    |provider_2.endSessionEndpointEnabled | true |
    |provider_3.endSessionEndpointEnabled | true |
    |provider_4.endSessionEndpointEnabled | true |
    ...
    |provider_1.endSessionRedirectUrl | https://{connections_server}/homepage |
    |provider_2.endSessionRedirectUrl | https://{connections_server}/homepage |
    |provider_3.endSessionRedirectUrl | https://{connections_server}/homepage |
    |provider_4.endSessionRedirectUrl | https://{connections_server}/homepage |
    |jndiCacheName | services/cache/OpenidRpCache |

    !!! note
    
        Endpoint configuration now uses `discoveryEndpointUrl` as the primary source. In earlier releases, `authorizeEndpointUrl`, `jwkEndpointUrl`, and `tokenEndpointUrl` had to be defined explicitly. These endpoints are now automatically resolved through the discovery endpoint.

        The `endSessionRedirectUrl` value shown in this table is an example. Set this value to the URL where you want users redirected after sign-out.

11. Create a new object cache instance with the JNDI name matching the one used in the last TAI property in the preceding table.
    1. Go to **Resources** \> **Cache instances** \> **Object cache instances**\> **New**.
    2. Set the **name** to *oidc_cache*
    3. Set the **JNDI name** to <filepath>services/cache/OpenidRpCache</filepath>
    4. Select **Enable cache replication**
    5. Set the **Full group replication domain** to *ConnectionsReplicationDomain*
...
15. Change the security role mapping for users/groups for applications:
...
16. Verify RichTextEditors application role mapping (if applicable):

    If RichTextEditors is installed in your environment, verify the application's role mapping after enabling Keycloak OIDC to avoid unintended editor access issues after migration:

    1.  Click **Applications** \> **Application types** \> **Enterprise Applications** \> **RichTextEditors** \> **Security role to user/group mapping**.

    2.  Select the **Everyone** role and set its mapping to **None**.

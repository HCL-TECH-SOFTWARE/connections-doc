...
- Configure a hardcoded `realmName` claim through the client scope mapper property.
...
8.  Fill in the following fields with the values below and click **Save**.
    ...
    ![keycloak_config_oidc_prov_p3.jpg](keycloak_config_oidc_prov_p3.jpg)
...
11. **[Optional]** Create additional clients for the Connections mobile and desktop plugins applications. Similar to creating the main Connections client, in the **Keycloak admin portal** go to  **Clients** > **Create**. Repeat for each client.    
   ...
   - Add another Keycloak client with Client ID: `conn-dsk-plugin`
   - Other than the Client ID, use the same settings as the mobile Keycloak client.
   - Add the following redirect URI to the **Valid Redirect URIs** list of the `conn-dsk-plugin` client:
...
####  Outlook Add-in: 
   ...
   - Update the **Web Origins** field to allow CORS origins from the redirect URIs. Either explicitly specify each redirect URI or specify `+` to allow all redirect URIs from the **Valid Redirect URIs** field.

   - Update `{realm}` settings to ensure `X-Frame-Options` includes `frame-ancestors 'self'`. This allows the Outlook Add-in to be loaded in an iFrame.

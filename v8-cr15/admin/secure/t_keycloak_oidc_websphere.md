...
1.  For every cluster, deploy the [WebSphereOIDCRP](chatgpt://generic-entity?number=0) application. To install WebSphereOIDCRP, follow the instructions in the WebSphere documentation:

    [oai_citation:0‡ibm.com](https://www.ibm.com/docs/en/was-nd/8.5.5?topic=users-configuring-openid-connect-relying-party&utm_source=chatgpt.com) and follow *Step 1* in *“Install the OpenID Connect application“*.
...
2.  Apply a unique context root to each cluster. For example, for the Apps cluster, go to **Enterprise Applications** > **WebSphereOIDCRP_AppsCluster.ear** > **Context Root For Web Modules**.
...
4.  Configure Global Security Custom Properties by clicking **Security** > **Global security** > **Custom Properties** and setting the following properties to the specified values:
...
5. Since we are using Keycloak clients for authentication, disable the **com.ibm.ws.security.oauth20.tai.OAuthTAI** filter.
    ...
    As the WebSphere administrator, in the administrative console, go to **Security** > **Global security** > **Web and SIP security** > **Trust association** > **Interceptors**. Select *com.ibm.ws.security.oauth20.tai.OAuthTAI* and set the **provider_1.filter name** to a dummy value to disable request interception.
...
10.  Configure the Keycloak realm as a trusted realm:
    ...
    3. For the **External realm name** field, enter the value for `{realm}`.
    4. Click **OK**.
...
11. Create a new object cache instance with the JNDI name matching the one used in the last TAI property in the preceding table.
    ...
    1. Go to **Resources** > **Cache Instances** > **Object cache instances**.
    2. Set the **name** to *oidc_cache*.
    3. Set the **JNDI name** to `services/cache/OpenidRpCache`.
...
12. Go to **Application servers** > **cluster** > **Container Services** > **Dynamic cache service**, and for each cluster’s dynamic cache service, ensure that cache replication is enabled and uses **ConnectionsReplicationDomain**.
...
14. Add the root signing certificate of the Microsoft Certificate endpoint to the default trust store:
    ...
    7.  For the **Alias** field, enter `cnxoidccert`. (You used this name as the value for `property provider_1.signVerifyAlias`.)

...
We need to update the `LotusConnections-config.xml`, the `service-location.xsd`, and the `opensocial-config.xml` and `mobile-config.xml` (for Desktop Plugin and Mobile Client).

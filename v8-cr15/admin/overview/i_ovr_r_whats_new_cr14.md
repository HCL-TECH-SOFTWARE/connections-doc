...
## AWS Application Load Balancer support {#section_alb_support .section}

For deployments on AWS, an Application Load Balancer (ALB) can now be used as an alternative to NGINX for routing traffic to Customizer, Grafana, Collabora, and Connections, with full support for path-based routing, WebSocket upgrades for Collabora, and DNS setup through Route 53.

For details, see [Setting up an AWS Application Load Balancer](../../admin/install/cp_config_setup_alb.md).
...
## Continuous scrolling in Files configuration

By default, the Files application uses continuous scrolling to load content as users scroll down the page. Administrators can now configure this setting to enable or disable continuous scrolling based on user preferences and performance considerations.

Refer to [Disabling continuous scrolling in Files](../../admin/admin/files_t_disable_continuous_scrolling.md) for details.

...
8.  Perform the following steps to grant access privileges for the applications:
    ...
    2.  Enter the following command:
        ...
        where `admin_user` and `admin_password` are the credentials for the user ID that you created in a previous step or an existing ID with administrative privileges.
        ...
        where `password_for_application_user` is the password for application database \(`FNGCDUSER` and `FNOSUSER`\).

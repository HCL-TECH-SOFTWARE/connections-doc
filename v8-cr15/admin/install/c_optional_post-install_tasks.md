...
Assuming that you have completed the server setup previously described, to enable locked domains in HCL Connections, specify an additional attribute in the LotusConnections-config.xml to ensure that only the Connections OpenSocial application is mapped to the locked domain host.
...
To extend HCL Connections with real-time document editing and collaboration, install HCL Enterprise Edition Collabora Online using the umbrella Helm chart from the HCL Harbor OCI registry.

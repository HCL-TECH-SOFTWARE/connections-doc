...
The OpenSocial API, by default, only supports users posting against its own activity stream. So, to enable the dedicated Share functionality, we use the ICXT proxy API. This API impersonates the authenticated user and executes the XHR request against the OpenSocial API in the name of a pre-configured Connections admin user. The admin user is configured by default during the ICXT installation, so there is no additional step required on the Share functionality side.
...
Payload is calculated on the client side and passed through to the OpenSocial API by ICXT back-end code. This is why you will not see any requests against the OpenSocial API in the browser network console. The client side of Share Dialog code only interacts with the ICXT proxy API.
...
-   **Payload include-files** - a custom JavaScript file that is injected into the HTML page by Customizer

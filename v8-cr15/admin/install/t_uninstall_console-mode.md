...
8.  If you plan to reinstall HCL Connections™, remove the following files:
    ...
    4.  Connections configuration files: Delete the [profile\_root](../plan/i_ovr_r_directory_conventions.md)`/config/cells/cell_name/LotusConnections-config` directory, where `cell_name` is the name of your WebSphere Application Server cell.

    5.  If it is present, delete the `registry.xml` file from the [profile\_root](../plan/i_ovr_r_directory_conventions.md)`/config/cells/cell_name` directory.

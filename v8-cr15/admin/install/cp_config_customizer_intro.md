...
For an example showing how to configure NGINX as a reverse proxy for Customizer, see [Configuring the NGINX proxy server for Customizer](cp_config_customizer_setup_nginx.md). If your deployment runs on AWS, you can alternatively use an Application Load Balancer (ALB) instead of NGINX; see [Setting up an AWS Application Load Balancer](cp_config_setup_alb.md).
...
Configure a service principal name and keytab using HCL Connections Customizer in an SPNEGO environment.

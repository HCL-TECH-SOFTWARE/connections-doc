...
The [Kubernetes Metrics Server](https://github.com/kubernetes-sigs/metrics-server) collects CPU and memory metrics from each node and exposes them through the Kubernetes API. 

Although optional for the core Component Pack functionality, it is required for the [Horizontal Pod Autoscaler (HPA)](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/) to function. Without it, HPA cannot retrieve resource metrics and pods will not autoscale.

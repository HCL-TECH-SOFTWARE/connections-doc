...
2.  Edit the `widgets-config.xml` file. Find the `<resource>` element with the type of community (`<resource ... type="community" ...>`), then under `<widgets>`, and within `<definitions>`, add the following lines, replacing `[BOARDS_URL]` with your URL:

    ```sh   
    <!-- Kudos Boards -->
    <widgetDef defId="Kudos Boards" modes="view fullpage"
    url="{webresourcesSvcRef}/web/com.ibm.social.urliWidget.web.resources/widget/urlWidget.xml"
...
4.  AAdd a mapping for the Activities Plus service to the Huddo Boards (formerly Kudos Boards) client. Ensure the OAuth Client is set to `conn-ee` and the Service name is **Kudos Boards**. Click the **OK** button.

5. Enter the following, replacing `[BOARDS_URL]` with your URL:

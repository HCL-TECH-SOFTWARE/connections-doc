...
## Collabora Online {#collabora_online_https .section}

After installing Collabora Online \(see [Installing Collabora Online](t_inst_collabora_online.md)\), add the following rules to your httpd.conf on your IBM HTTP servers and restart the service.

IBM HTTP Server handles the HTTP paths (`/browser`, `/hosting`, `/cool`, `/controller`). WebSocket traffic (`/cool/.../ws` and `/controller/.../ws`) is handled by NGINX — see [Configure NGINX (WebSocket proxy)](t_inst_collabora_online.md#section_config_nginx_collabora) for details.

In the rules below, replace the scheme and port to match your Component Pack ingress: use `http` with NodePort `32080` when the Component Pack ingress is HTTP, or `https` with NodePort `32443` when TLS is enabled on the Component Pack ingress.

``` {#codeblock_collabora_online_proxy}
# proxy rules for Collabora Online

# Collabora needs encoded slashes in URLs to pass through unmodified
AllowEncodedSlashes NoDecode

ProxyPass        "/browser"    "http://cpmaster.internal.example.com:32080/browser" retry=0
ProxyPassReverse "/browser"    "http://cpmaster.internal.example.com:32080/browser"
ProxyPass        "/hosting"    "http://cpmaster.internal.example.com:32080/hosting" retry=0
ProxyPassReverse "/hosting"    "http://cpmaster.internal.example.com:32080/hosting"
ProxyPass        "/cool"       "http://cpmaster.internal.example.com:32080/cool" retry=0
ProxyPassReverse "/cool"       "http://cpmaster.internal.example.com:32080/cool"
ProxyPass        "/controller" "http://cpmaster.internal.example.com:32080/controller" retry=0
ProxyPassReverse "/controller" "http://cpmaster.internal.example.com:32080/controller"
```

If your IBM HTTP Server enforces a `Content-Security-Policy` header, ensure your external HCL Connections FQDN is allowed alongside `'self'`, for example:

```
Header always set Content-Security-Policy "... 'self' https://<application_load_balancer_fqdn> ..."
```

Be sure that you have WebSockets enabled on your front proxy server, because Collabora Online requires WebSocket connections for real-time collaboration.

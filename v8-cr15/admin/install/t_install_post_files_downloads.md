...
2.  Locate and copy the redirect module file:

    **For IHS 8:**
    
    a.  On the server where you installed HCL Connections (on the deployment manager), navigate to the [`connections_root`](../plan/i_ovr_r_directory_conventions.md)/plugins/ihs/mod_ibm_local_redirect/platform directory to find the module file (`mod_ibm_local_redirect.so`). On supported operating systems, search the following directories:
    ...
    b. Copy the module to the appropriate directory on the system that hosts IBM HTTP Server. By default, modules are stored in the [ibm\_http\_server\_root](../plan/i_ovr_r_directory_conventions.md)/modules directory.

    **For IHS 9:**
    
    a.  Verify that the IHS 9 plug-in installation kit is installed and includes the appropriate module file.

    b.  The module file named **mod\_ibm\_local\_redirect.so** should be in the IHS modules extra folder, by default  [ibm\_http\_server\_root](../plan/i_ovr_r_directory_conventions.md)/modules/extra. If it is not there, copy it from the install location to the extra folder.

3.  Open the `httpd.conf` file \(in the [ibm\_http\_server\_root](../plan/i_ovr_r_directory_conventions.md)/conf directory by default\) and add the following statements to load the `ibm_local_redirect_module`, and the `mod_env` environment variable module:
    ...
    **For IHS 9:**
    ...
    For example: `LoadModule ibm_local_redirect_module modules/extra/mod_ibm_local_redirect.so`
...
4.  Grant access to the data directory root:
    ...
    !!! note
        ...
        -   If the Connections Mobile service is installed, you must also give IBM HTTP Server access to the FileDiff StoragePath. You can find the FileDiff StoragePath attribute in the FileDiff section of the `mobile-config.xml`file. For information about editing the `mobile-config.xml` file, see the *Changing configuration property values* topic. This attribute contains either the path itself, or a WebSphere variable whose value is the path. For example, if the value of the variable is `${MOBILE_CONTENT_DIR}`, examine the `MOBILE_CONTENT_DIR` variable in the WebSphere Application Server console to find the path. For more information about WebSphere variables, see the *Changing WebSphere Application Server environment variables* topic.
...
5.  On all virtual hosts in the same domain as Activities, Files, Mobile, or Wikis, including both HTTP and HTTPS, expose the data directory root:
    ...
    2.  To create an alias for the data directory root, add the following line:
        ...
        ```sh
        alias /alias "[data_directory_root](../plan/i_ovr_r_directory_conventions.md)"
        ```
...
6.  To make the alias more secure, add the following lines to the `httpd.conf` file, adding them after the lines that you added in Step 6. Note that the configuration steps for IHS 8 differ from steps for IHS 9.
    ...
    **For IHS 8:**
    ...
    <Directory "[data\_directory\_root](../plan/i_ovr_r_directory_conventions.md)">
      Order Deny,Allow
      Deny from all
      Allow from env=REDIRECT\_FILES\_CONTENT or REDIRECT\_MOBILE\_CONTENT or REDIRECT\_WIKIS\_CONTENT
    </Directory>
    ```
    For example:
    ...
    **IHS 9 Steps**
    ...
    ```sh
    <Directory "[data\_directory\_root](../plan/i_ovr_r_directory_conventions.md)">
        Require env REDIRECT\_FILES\_CONTENT or REDIRECT\_MOBILE\_CONTENT or REDIRECT\_WIKIS\_CONTENT
    </Directory>
    ```
    For example:
    ```sh
    <Directory "/opt/IBM/Connections/activities/content">
       Require env REDIRECT_ACTIVITIES_CONTENT
    </Directory>
    ...
    <Directory "/opt/IBM/Connections/data/shared/mobile">
       Require env REDIRECT_MOBILE_CONTENT
    </Directory>
    
    <Directory "/opt/IBM/Connections/data/shared/wikis">
       Require env REDIRECT_WIKIS_CONTENT
    </Directory>
    ...
    !!! note
        ...
        -   This definition secures the data by allowing requests where `REDIRECT_FILES_CONTENT` or `REDIRECT_MOBILE_CONTENT` or `REDIRECT_WIKIS_CONTENT` only is specified. Use any environment variable that you want, provided it is not already in the IBM HTTP Server environment.
...
7.  To enable the modules for Activities, Files, Mobile, and Wikis, add the following lines to the httpd.conf file, adding them after the lines that you added in Step 6:
    ...
    !!! note
        ...
        -   For mobile, the `application_context_root` is the FileDiff context root; by default, this root is `/connections/filediff`.
        ...
        -   The SetEnv value sets the token that the data directory requires to be accessible. It must match the value after REDIRECT\_ that you set in `Allow from env=` in Step 6. For example, if you set REDIRECT\_FILES\_CONTENT in Step 6, this value must be SetEnv FILES\_CONTENT true.
...
8.  Test that IBM HTTP Server is configured properly and securely:
    ...
    2.  Try to access the alias directory directly at http/https:host/alias and make sure that you are denied permission. If you can access the directory, make sure that the `Order Deny, Allow; Deny from All; Allow from env` from Step 6 are all present.

    3.  Access the application and download a file to make sure that it functions. The module is not yet enabled.

9. Check out the `files-config.xml`, mobile-config.xml, `oa-config.xml`, or `wikis-config.xml` file by using the steps in the topic *Editing configuration files*, and specifying the following property attributes:
    ...
    !!! note
        ...
        -   For Activities, you must add separate `<download>` elements to each `<store>` element in `oa-config.xml`. Each `<download>` element references an Alias defined in Step 5.
...
10. Restart Activities, Files, Mobile, or Wikis, depending on which applications you are configuring.

11. Download a file to make sure that the new configuration works.

12. Test whether IBM HTTP Server is downloading files:
    ...
    1.  Open the httpd.conf file and add \# characters to comment out the last line in the <Directory\> element,
        ...
        **IHS 9 Example:**
        ...
        ```sh
        <Directory "[data\_directory\_root](../plan/i_ovr_r_directory_conventions.md)">
            #Require env REDIRECT_FILES_CONTENT or REDIRECT_MOBILE_CONTENT or REDIRECT_WIKIS_CONTENT        
        </Directory>
        ```
        For example:
        ```sh
...
13. Ensure that Activities is downloading files from the HTTP server:
    ...
    3.  You should see a line like the following in the trace.log file: 

      ```sh
      [2/5/13 16:16:18:134 EST] 00000090 OaDownloadAct 1 sun.reflect.GeneratedMethodAccessor86 invoke OaDownloadAction setting X-IBM-Local-Redirect header to : /activities_content/122/110/69aed142-0b89-4a7a-985a-cfd99cca1292
      ```

...
!!! Note
    ...
    An updated HCL Connections V8.0 Wizard that supports Microsoft SQL Server 2025 and the JDBC 13.4 driver is available on My HCLSoftware.
...
11. Enter the following information about the database, and then click **Next**:
    ...
    - JDBC driver library path
        ...
        :   Download the SQL Server JDBC 13.4 driver from the [Microsoft JDBC Driver for SQL Server ](https://learn.microsoft.com/en-us/sql/connect/jdbc/release-notes-for-the-jdbc-driver?view=sql-server-ver16#134) website to a local directory and enter that directory name in the **JDBC driver library** field.

        See **Working with the Population Wizard and Microsoft JDBC Driver 13.4 for SQL Server** at the bottom of this page for more information.
...
## Important: Working with the Population Wizard and Microsoft JDBC Driver 13.4 for SQL Server

When populating the PeopleDB using the Population Wizard with the Microsoft JDBC Driver 13.4 for SQL Server, the following error may be logged:

`Truststore file does not exist.`

### Workaround

1. Run the Population Wizard once to generate the required configuration files.
2. Navigate to the Population Wizard TDI directory. For example:
   `C:\Wizards\TDIPopulation\win\TDI`
3. Edit `solution.properties` and configure the following properties:
   - `javax.net.ssl.trustStore`
   - `javax.net.ssl.trustStoreType`
4. You do not need to set `javax.net.ssl.trustStorePassword`; the default value is used.
5. Leave the existing LDAP and database settings in `profiles_tdi.properties` unchanged.
6. Manually run the following command to complete the PeopleDB population:
   `TDIPopulation/win/TDI/populate_from_dn_file.sh`

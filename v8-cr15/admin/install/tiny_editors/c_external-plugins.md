...
The Tiny Editors integration also provides the ability to selectively load these external plugins based on the editor location. For example, you can load a track-changes plugin only on Wiki pages where multiple users can collaborate. The plugins can be selectively turned on in certain locations or simply loaded everywhere except certain locations.
...
`settings`
:   is an optional JavaScript object with settings specific to the plugin which will be merged into the TinyMCE settings. Any settings already set by the integration cannot be overridden using this property.

`on`
:   is an optional list of strings containing [editor locations](r_plugins-locations.md) where the plugin will be loaded unless listed in `off`. If this setting is not provided then it will default to all locations.

`off`
:   is an optional list of strings containing [editor locations](r_plugins-locations.md) where the plugin will not be loaded. If this setting is not provided then it will default to no locations.

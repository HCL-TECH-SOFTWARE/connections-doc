...
The toolbar is the main interface of the editor. This section covers how the components of the toolbar relate to the toolbar definition in `config.js`.
...
The toolbar is constructed from four parts.
...
Custom groups can be named or anonymous. Any object in the top-most array must be a named group. Any array in the top-most array must be an anonymous group. Named groups have a label and a list of items, while anonymous groups are just the list of items.
...
Custom menus always have their `id` and `items` properties. If the `id` does not match a predefined menu then the `label` and `icon` properties should be specified as well.

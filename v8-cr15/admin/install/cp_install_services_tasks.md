...
**Let's make a few assumptions about the environment:** We are installing an environment that will be available behind `nginx.example.com`. However, this is just a public domain.
...
- Kubernetes cluster is a single node and is running on `cpmaster.internal.example.com`.
- NFS master is collocated with `connections.internal.example.com`, and its IP address is `172.27.1.48`. All folders are created in `172.27.1.48:/pv-connections/`.
...
We don't recommend or support any particular configuration of NFS – You can use whatever NFS implementation is available. For the sake of this example, however, let's assume that our NFS master is on `connections.internal.example.com`, you have root access there, you installed NFS, you know how to manage it, and you just need the components needed for Component Pack.
...
1. Perform these steps on the NFS master:

    1. Create the `/pv-connections` folder on `connections.internal.example.com` with permissions `0700`. You may assign ownership to the user and group of your choice.

    2. Create a user with ID 1000 and another user with ID 1001, if they do not already exist. These users are used by OpenSearch and MongoDB to generate data.

    3. Inside the folder, create the following subfolders. You may set the ownership to your desired group while user ownership should be set to the user ID creating data during runtime.

        - `/pv-connections/customizations` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchbackup` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchmaster-0` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchmaster-1` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchmaster-2` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchdata-0` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchdata-1` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchdata-2` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchclient-0` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchclient-1` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/opensearchclient-2` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/kudos-boards-minio` — permissions ``0700`, owner uid `1000`  
        - `/pv-connections/mongo7-node-0/data/db` — permissions ``0700`, owner uid `1001`  
        - `/pv-connections/mongo7-node-1/data/db` — permissions ``0700`, owner uid `1001`  
        - `/pv-connections/mongo7-node-2/data/db` — permissions ``0700`, owner uid `1001`

    4. Download `nfsSetup.sh` and `volumes.txt` from the HCL Connections deployment automation repository to a directory of your choice (for example, `/tmp`).

    5. Make `nfsSetup.sh` executable and run it. Then complete the NFS configuration:

        1. Install required NFS packages (if not yet installed by default).
...
2. Configure NFS clients by enabling and starting the NFS service on all Kubernetes master and worker nodes:

    ```bash
    systemctl restart nfs-server
    ```

Ensure that all persistent volumes are exported and mountable from Kubernetes masters and worker nodes before proceeding.
...
4. Back up all Elasticsearch 7 indexes by running the following command:
      ...
      Where `${REPONAME}` is the name of the snapshot repository that was previously used to register and manage the Elasticsearch 7 snapshot, for example, `connectionsbackup`.
...
3. Similarly, with MongoDB 7, you need persistent volumes (PVs) for all replicas of the `mongo7` pod. Create additional MongoDB volumes on the NFS master node. The folders should be owned by user ID `1001`, while you may set the ownership to your desired group:
...
!!! note
    ...
    - In your `connections-env.yml`, set the `namespace` parameter (*__default_namespace*) to `connections`. In a highly available Kubernetes environment, set `replicaCount` (*__replica_count*) to `3`.
...
3. Install the `connections-volumes` chart.

      FFind out the `connections-volumes` chart version that is available on Harbor OCI:
...
3. For example, if you're configuring etcd volumes for 3 replicas, you'll need to replace the variables with your actual values:
      ...
      - `pv_name`: The name of the persistent volume (for example, `<<namespace>>-etcd`)
      - `item`: The replica number (`0`, `1`, or `2`, because the replica count is `3`)
      - `__apisix_storage_class_name`: The name of your storage class (for example, `<<namespace>>-storage-class`)
      - `__apisix_volume_storage_capacity`: The storage size (for example, `10Gi`)
      - `__apisix_volume_access_mode`: The access mode (for example, `ReadWriteOnce`)
      - `__apisix_storage_class_reclaim_policy`: The reclaim policy (for example, `Retain`)
      - `__persistentVolumePath`: The path on the NFS server (for example, `pv-connections`)
      - `__nfsMasterAddress`: The IP address of the NFS server
...
During the bootstrap installation, secrets are created or overwritten under the following conditions:
...
The `configmap` for `connections-env` contains all the variables required for Customizer and Orient Me to function properly. Note that Customizer always points directly to the IBM HTTP Server, whereas Orient Me requests are routed through the front-door proxy.

1. FFind out the `connections-env` chart version that is available on Harbor OCI:
...
4. Remove the `cnx-ingress-appreg` and `cnx-ingress-orient-me` ingress resources:
...
2. Download the custom values template `cnx-ingress-traefik-values.j2` from the [HCL Connections deployment automation Git repository](https://github.com/HCL-TECH-SOFTWARE/connections-automation/tree/main/roles/hcl/component-pack-harbor/templates/). **Always download the latest version of this template** to ensure compatibility with the current release. Rename it to `cnx-ingress-traefik-values.yml` and then update it with your environment-specific values.

3. (Upgrade only) Update the Traefik CRDs. Helm does not automatically update CRDs on upgrade, so run this command before installing:

      ```bash
      helm show crds traefik/traefik --version 41.0.2 | kubectl apply --server-side --force-conflicts -f -
      ```

4. Install Traefik:

      ```
      helm upgrade cnx-ingress traefik/traefik -i --version 41.0.2 --namespace connections -f cnx-ingress-traefik-values.yml --wait --timeout 10m
      ```

5. Wait for Traefik pods to be ready

      ```
      kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=traefik --namespace connections --timeout=300s
      ```

6. Verify Traefik service is created:
...
**Upgrading MongoDB 7: Managing the Security Job**

When upgrading the Component Pack from CR14, the existing security Job must be cleared out. Because **mongo7** is a sub-chart within the **infrastructure** Helm chart, clearing this Job allows the new **infrastructure** chart to execute its MongoDB security checks cleanly during the upgrade.

1. Check the Status of the Existing Job:

      Verify if the check-and-update-mongo7-security job currently exists in your Connections namespace.

      ```
      kubectl get jobs --namespace connections | grep check-and-update-mongo7-security
      ```

2. Delete the Existing Job:

      If the Job exists, delete it. Deleting the Job removes the immutable specifications and allows the new infrastructure Helm chart (and its updated **mongo7**  sub-chart) to deploy without throwing a patch error.

      **Note**: This will not delete your MongoDB data or affect the running **mongo7** replica sets.

      ```
      kubectl delete job check-and-update-mongo7-security --namespace connections
      ```

**Starting with v8 CR13**, Redis has been replaced by Valkey. It is recommended to remove any existing Redis resources before upgrading the infrastructure chart. Although the infrastructure chart is designed to clean up old resources automatically, running the following `kubectl delete` commands beforehand can help ensure a smooth upgrade process.
...
!!! note 
    ...
    If you are upgrading from v8 CR11 or earlier, see the [Delete legacy Mongo security check job and Redis resources](#del_legacy_redis) step before upgrading the infrastructure chart.
...
5. Set up your reverse proxy to forward some traffic to the customizer – see [Configuring the HTTP server](cp_config_proxy_rules.md).
    ...
    !!! note
        ...
        After you configure the reverse proxy, enable the app registry service in Connections so Customizer can discover its extensions. If this service is not enabled, Customizer integration will not work as expected. Follow the steps in [Enabling the Connections app registry service for customization](../customize/enabling-app-registry.md).
...
2. Download the J2 templates for `opensearch_master.yml`, `opensearch_data.yml`, and `opensearch_client.yml` from the [HCL Connections deployment automation Git repository](https://github.com/HCL-TECH-SOFTWARE/connections-automation/tree/main/roles/hcl/component-pack-harbor/templates/helmvars) and modify them according to your environment.

3. Before you start deploying opensearch chart, we recommend that you override the default password provided in the script. You can provide this user-defined password by adding or updating the following variable in the `opensearch_master.yml`, `opensearch_data.yml`, and `opensearch_client.yml` files:
...
8. This step is optional. Refer to the [Voting configuration exclusions API](https://www.elastic.co/guide/en/elasticsearch/reference/current/voting-config-exclusions.html).
      ...
      By default, this chart installs three master nodes. If you want to reduce the number of master nodes to one during deployment, you can use the `voting_config_exclusions` API. This API allows the voting configuration to be reduced to fewer than three nodes, or more than half of the master-eligible nodes to be removed from the cluster at once, by manually removing departing nodes from the voting configuration.

      For each specified node, the API adds an entry to the cluster's voting configuration exclusions list. It then waits for the cluster to reconfigure its voting configuration to exclude the specified nodes.

      For example, to add the nodes `opensearch-cluster-master-1` and `opensearch-cluster-master-2` to the voting configuration exclusions list:
...
- Infrastructure charts must already be installed and all pods scheduled.
- The `connections-env` ConfigMap must already be present.
- OpenSearch must be installed and running.
- After installing Orient Me, you must run the Profiles migration. In this step, users are migrated from PeopleDB to MongoDB, which is used by Orient Me.
- Rewrite rules must be configured in `httpd.conf` on the IBM HTTP Server to allow requests from Connections.
...
6. To migrate profiles, run this:
      ...
      IIf you followed the examples and the installation order, this should work out of the box. If you want to make customizations, or if you are later changing the database, you can open an interactive terminal session inside the `people-migrate` pod to view the configuration that was pre-created for you during the installation or upgrade of Component Pack in `/usr/src/app/migrationConfig`.
...
3. Copy the certificate files to the WebSphere Deployment Manager in a common location that is readable and writable by all WebSphere Application Server nodes.
      ...
      For example, copy the two certificate files created in step 1 (that is, `/tmp/es_certs/chain-ca.pem` and `/tmp/es_certs/opensearch-metrics.p12`) to the following directory: `/opt/IBM/es_certs` on the WebSphere Deployment Manager.
...
4. Configure OpenSearch metrics within Connections:
      ...
      5. Enable or switch to OpenSearch Metrics. The following script causes the RDBMS-based app to stop capturing data, and the OpenSearch component to start capturing it.
         ...
         1. On the WebSphere Deployment Manager, open `wsadmin`, making sure to use the `-lang jython` option. For example, on Linux, run the following commands to start `wsadmin`:
...
5. Manage the Elasticsearch index for Connections type-ahead search. The type-ahead search feature uses an index named “quickresults” within the OpenSearch search engine.
      ...
      1. On the WebSphere Deployment Manager, open `wsadmin`, ensuring that you use the `-lang jython` option. For example, on Linux, run the following commands to start `wsadmin`:
      ...
      2. Merge the Signer certificate into the opensearch-metrics.p12 keystore:
         ...
         Disconnect from the `wsadmin` environment by entering the `quit` command.

      3. Copy the updated `opensearch-metrics.p12` file from the Deployment Manager to the same location on the WebSphere Application Server nodes.
   
      4. Download `config_blue_metrics.py` from the [HCL Connections deployment automation Git repository](https://github.com/HCL-TECH-SOFTWARE/connections-automation/tree/main/roles/hcl/component-pack-harbor/files/config_blue_metrics.py). This script sets the OpenSearch server base URL in Metrics.
...
#### Teams Share or Messaging Extension
...
2. Download the Jinja2 (j2) template for `tailoredexperience.yml` from the following location: [HCL Connections deployment automation Git repository](https://github.com/HCL-TECH-SOFTWARE/connections-automation/tree/main/roles/hcl/component-pack-harbor/templates/helmvars) and modify it according to your environment.
...
2. Load the HCL Connections configuration file:
...
3. Check out the HCL Connections configuration files:
...
4. Update the add-in Docker environment variables, which are located in the `outlook-addin.yaml` file. These are passed into the Outlook add-in Docker instance on startup:
      ...
      - What must be overriden:
         - `CONNECTIONS_URL` - URL of your Connections environment without a trailing slash (for example `https://my.connections.server.com`). The same URL has to be used when generating the secret in the first step.

- `CONNECTIONS_CLIENT_SECRET` - Client secret generated by Connections when registering the OAuth provider in the first step.
- `CONNECTIONS_CLIENT_ID` - Client ID (aka app ID) used when registering the OAuth provider in Connections in the first step (default: `hcl-cnx-office-addin`)
      ...
      - What may be overriden:
         ...
         - ``SUPPORT_URL` - URL that a user can go to for support (help). (default: https://opensource.hcltechsw.com/connections-doc/connectors/enduser/c_ms_plugins_add_in_outlook.html)

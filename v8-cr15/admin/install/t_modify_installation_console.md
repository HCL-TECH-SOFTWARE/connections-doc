...
10. Enter the database information:
    ...
    6.  Ensure that the following JDBC driver libraries are present in the JDBC directory:
        ...
        - SQL Server:    The SQL Server JDBC 4 driver name is hard-coded in the Connections 8.0 installation. The SQL Server JDBC 13.4 driver can be used if it is renamed to the expected driver name, sqljdbc42.jar.   
        
            The JDBC driver can be download from the [Microsoft JDBC Driver for SQL Server ](https://learn.microsoft.com/en-us/sql/connect/jdbc/release-notes-for-the-jdbc-driver?view=sql-server-ver16#134) website to a local directory and enter that directory name in the **JDBC driver library** field.

            The directory only have the only the sqljdbc42.jar file. An exception occurs if more than one jar file are present in the same directory.

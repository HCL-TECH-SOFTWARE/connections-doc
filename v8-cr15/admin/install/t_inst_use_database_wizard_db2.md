...
!!! note
    -   If you are using DASD driver, the SQL scripts are located in the    `connections.s390.sql/application_subdirectory` directory of the HCL Connections setup directory or installation media.
...
!!! attention 
    HCL Connections™7.0 install wizard does not create the database for the Community Highlights / Connections Engagement Center \(ICEC\) application. You will need to manually run the SQL scripts that are provided with HCL Connections. See [Creating IBM DB2 databases manually](t_install_dbs_db2.md) to create the ICEC database.

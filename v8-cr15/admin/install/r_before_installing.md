...
To support the enhanced security model in SQL Server 2025 and the JDBC 13.4 driver:

* Export the SQL Server certificate and install the issuing CA certificate in the **Trusted Root Certification Authorities** certificate store. Refer to [Export and import certificates](https://learn.microsoft.com/en-us/sql/database-engine/configure-windows/certificate-procedures?view=sql-server-ver17) for details
* Import the SQL Server certificate into the **WebSphere Application Server** trust store.
* Import the SQL Server certificate into **IBM Tivoli Directory Integrator (TDI)**.
* Configure the JDBC connections to use **TLS** and the trusted certificates.

...
5.  To include user profile images in enhanced email notifications, add or update the following property:
    ...
    !!! note 
        ...
        The default value is `false`. When this property is set to `false`, enhanced email notifications use the stock application image instead of a user's profile image.
...
Easily customize several notification properties \(such as company name/logo, sender name/email address, and profile image rendering\) from one central location. For more information, see [Customizing basic notification settings \(CR4 and later\)](../customize/t_customize_new_template.md).

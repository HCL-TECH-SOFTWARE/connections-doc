...
A community template is a copy of certain aspects of an existing community \(such as the layout and some apps, including some of their content\). To create a template, you will need the URL of the source community. You can associate the new template with categories that you can filter on later to choose an appropriate template on which to base a new community.
...
Later, when a community is created from the template, the creation dates of the contents are what they were in the template, but the "last modified" dates are updated with the time that the community is created.

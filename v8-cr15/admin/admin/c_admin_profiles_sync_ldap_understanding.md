...
The second phase iterates through the source, typically the LDAP, with the same progress indicators as Phase 1. Each employee is added to a partition file. The partition files are named sequentially from `0.ldiff` up to `9.ldiff`, assuming the default value of 10 for `sync_updates_hash_partitions`. Each .ldiff file is essentially a collection of ldiffs, each of which contains all the LDAP data per employee.

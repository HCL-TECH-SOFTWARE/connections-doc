...
Perform the following steps to replace URLs in bookmarks:
...
3.  Run the following command to replace an existing URL with a new URL you specify:
    ...
    - `log_file`
    :   A local file path that is used to log any messages that result from running the command. The file is created if it does not exist; otherwise, new log data is appended to the data in the existing file.
...
4.  To find URLs to replace and write them to a file, enter this command:
    ...
    - `base`
    :   A plain text string that is used to limit the number of URLs to be scanned. Only URLs containing the `<base>` string are scanned.

    - `URL_pattern`
    :   A Java-style regular expression that is used to match URLs.

    - `replace_str`
    :   A plain text string that is used to replace the found pattern in the matched URL.

    - `output_file`
    :   A local file path that is used to store the matched URLs and their replacement URLs. The file is created if it does not exist and is appended if it already exists.

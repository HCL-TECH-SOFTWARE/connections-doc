...
4.  Use the following commands to reparent communities or subcommunities:
    ...
    - `CommunitiesService.moveSubcommunityToCommunity("subCommunityUuid")`
        ...
        !!! note 
            ...
            You can get the UUID for a community or subcommunity by taking one of the following actions:
    ...
    - `CommunitiesService.moveCommunityToSubcommunity("parentCommunityUuid", "communityToMoveUuid")`
        ...
        -   The new subcommunity with existing web address is cleared.
            ...
            Any "Invited" users on the subcommunity that have not accepted or declined the invitation are removed. Click the **Invitations** tab on a community. You can see the invited users on the Members page.

        When you use `moveCommunityToSubcommunity`, the new subcommunity retains the following characteristics of the original top-level community:
    ...
    **Community access levels:** A subcommunity cannot be less restrictive than its parent. Therefore, when you run `moveCommunityToSubcommunity`, the access level of the community that is being moved to a subcommunity can be modified as follows:
    ...
    |Community A \(parent\)|Community B \(befoe it is made a subcommunity of A\)|Community B \(after it is made a subcommunity of A\)|
    |----------------------|-----------------------------------------------------|----------------------------------------------------|
    ...
    Restricted \(unlisted\) communities are those where the community owner has left **Let anyone in my organization see the community's title, description, tags, and owners** unchecked when creating the community.

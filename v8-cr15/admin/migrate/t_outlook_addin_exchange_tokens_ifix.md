...
For this upgrade, there are two environment variables that are needed to be added: `ENTRA_CLIENT_ID` and `ENTRA_TENANT_ID`. These variables can be found during the ***Update the Microsoft App Registration*** step. These can be updated within the helm chart or manually as described below:

...
1. Open a command window and execute the following script to check out the `LotusConnectionsConfig.xml` file. Leave the command prompt open for the next step.

    !!! tip
        Alternatively, you can modify the `LotusConnectionsConfig.xml` file manually and then synchronize the nodes in the WebSphere Integrated Solutions Console.
...
2. Open the checked-out `LotusConnectionsConfig.xml` file in an XML editor of your choice and add the `widgetBundle` section to the resources node, as shown in the following image.

    ![image](images/image79.png){width=800px}
...
3. Execute the following scripts:
    ...
    - `LCConfigService.checkInwidgetsConfig("<working_directory>","<cell_name>")`

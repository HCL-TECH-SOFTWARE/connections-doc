...
2.  Open the just checked out widgets-config.xml file in an XML editor of your choice and add the following `widgetDef`code to the resource tag containing `type="community"...>`.
    ...
    !!! note 
        ...
        If you are using Connections V5.5 with no Cumulative Releases applied, then you must install the following iFix: 5.5.0.0-IC-Container-IFLO88423.jar. For information on installing an iFix, see [Installing interim fixes in interactive mode](../../admin/migrate/t_install_interim-fix.md).

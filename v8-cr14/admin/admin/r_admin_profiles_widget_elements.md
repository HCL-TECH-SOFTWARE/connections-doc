...
|---------|-----------|--------|
|defId|The widget name, which must be unique. The defId attribute is also used as a title or a resource bundle key. This attribute takes a string value.|Yes|
...
|url|Specifies the location of the widget descriptor. This XML attribute can be parameterized with substitution variables. This attribute takes a string value.|Yes|
|modes|Specifies the modes that are supported by the custom widget. Possible modes include: view \(enables display on the profile overview page\), search \(integrates the widget into a community's search results page\), fullpage \(integrates the widget into the navigation bar\), and edit \(enables the **Edit** menu option and integrates the widget into the Edit Community page as a separate tab\).|No|
...
|pageId|Contains the page ID for the page that Profiles uses to render the widget layout. This attribute takes a string value. Possible values include profilesView, searchView, searchResultView, networkView, and editProfileView.|
|uiLocation|Specifies which column on the page contains the widget. This attribute takes a string value. Possible values include col1, col2, and col3 \(this option is not available for the networkView page\).|

...
The following extension property types are supported:
...
7.  To test, edit a user profile and set values for the person’s `preferredFirstName` and `preferredFrenchName`. In Profiles, click the **Directory** tab and perform a search on the person’s `preferredFirstName` and `preferredFrenchName` values.

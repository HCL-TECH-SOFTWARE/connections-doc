...
4.  Create a file in `IBM_Connections_Customization_Dir/strings` called `com.example.resources.customBundle.properties` and add the following key/value pairs.
...
13. Open the `installedApps/Profiles.ear/lc.profiles.app.war/WEB-INF/lib/lc.profiles.web.app.jar: com/ibm/lconn/profiles/strings/ui.properties` file. Extract the version of each resource file for each locale and save a copy for each locale by using the following sample as a guide.

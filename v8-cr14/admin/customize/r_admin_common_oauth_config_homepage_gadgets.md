...
HCL Connections™ supports an OAuth 2.0 consumer proxy that allows the Homepage component to surface gadgets in an OpenSocial container that can interact with an OAuth 2.0 protected service. In order for this proxy to function, there are a series of new administration commands that are exposed in the News service to define OAuth 2.0 providers, clients, and the associated gadget that interacts with the protected service.
...
-   [Registering or updating a provider](r_admin_common_oauth_config_homepage_gadgets.md#RegisteringOrUpdatingAProvider)
...
Returns a list of Map objects that represent each OAuth 2.0 provider registered with the consumer proxy, ordered by provider name in ascending order. The following information is returned for each map object in the returned list:
...
## Registering or updating a provider {#RegisteringOrUpdatingAProvider .section}
...
Returns a list of Map objects that represent each OAuth 2.0 client registered with the consumer proxy, ordered by provider name in ascending order. The following information is returned for each map object in the returned list:

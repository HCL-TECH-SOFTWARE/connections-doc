...
The set of properties references your supplied standard properties or custom extension properties.
...
-   Profiles defines a single base type of `snx:person` and `snx:mtperson` that enumerates the set of fields required on all profile records.
-   You can define subtypes of `snx:person` and `snx:mtperson` \(such as customer, employee, or contractor\) to add your own unique properties.
...
-   A profile-type declaration that omits a parentId implicitly inherits from `snx:person` and `snx:mtperson`.

    !!! note 
      
        There are important considerations when assigning profile type id values. These values appear as URL parameters in the Profiles API, and should accordingly facilitate valid URL encoding. The following rules help avoid encoding issues in the API.
...
**Sample – Profile-type declaration with hierarchy and inheritance**
...
The samples above define the profile-type identifier customer; this profile-type inherits from the system type `snx:person` and `snx:mtperson`. The customer type can add additional `<property/>` declarations that reference your standard properties or extension properties that were declared in the `profiles-config.xml` file.
...
|`updatability`|Enum|Indicates whether the value of this property can be updated. Options are: read – specifies that the property value cannot be modified using either a user interface or API element; this is applicable for a system property that is either maintained or computed by the application, Tivoli® Directory Integrator, or the administrator API. readwrite – specifies that the property value can be modified using either a user interface or API element.|
...
|`fullTextIndexed`|Boolean|If set to TRUE, and the property supports inclusion in the search index \(see [Standard properties in the data model](r_admin_profiles_attributes_std.md)\), the property is included in the search index for Profiles full text search. The default setting is TRUE for extension properties. If set to FALSE, the standard or extension property value is omitted from the search index for profile records with a matching profile-type. This property is enforced only if support for variable indexing of profile attributes is enabled. For more information, see [Specifying properties to expose in the search index](t_admin_profiles_expose_props_search.md).|
|`mapToNameTable`|Enum|Indicates whether this property value is intended as an additional given name or surname for use in a directory search from within the Profiles application. surname – Specifies that the value of this property is added as a surname to the Profiles database for use in a simple directory search. givenName – Specifies that the value of this property is added as a given name to the Profiles database for use in a simple directory search. none – Specifies that this value is not mapped to a name field in the Profiles database and is not used in a simple directory search. The default value for this property is none for all fields except the following: preferredFirstName - givenName; preferredLastName - surname.|
...
**Sample – Profile-type property definition**
...
The sample above adds the property identifier telephoneNumber to the associated profile-type.
...
To maintain compatibility with earlier versions, Profiles maintains the concept of a default profile-type.

...
2.  Use the wsadmin client to access and check out the Profiles configuration files.
    ...
    1.  Enter the following command to access the Profiles configuration files:
        ...
        If prompted to specify a service to connect to, type `1` to pick the first node in the list. Most commands can run on any node. If the command writes or reads information to or from a file by using a local file path, you must pick the node where the file is stored.

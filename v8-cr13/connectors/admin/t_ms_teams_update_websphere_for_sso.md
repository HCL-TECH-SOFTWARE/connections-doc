...
!!! important
    ...
    Like the chat integration for Teams, this SSO setup does require that the email address for the user's Teams identity matches the email address in Connections.
...
8.  Add the root signing certificate of the Microsoft Certificate endpoint to the Default Trust Store:
    ...
    !!! note
        ...
        If this certificate is missing or outdated, WebSphere cannot retrieve the JWKS signing keys from `login.microsoftonline.com`.  This failure results in the following error: `CWTAI2047E: No key was found to verify the signature`. Microsoft might rotate its TLS certificates periodically. If you encounter this error, repeat the previous steps to import the updated certificate. For information about this process, see [Azure TLS certificate changes](https://learn.microsoft.com/en-us/azure/security/fundamentals/tls-certificate-changes) in the Microsoft documentation.

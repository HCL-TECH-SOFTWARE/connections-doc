...
4. Secrets and Certificate Management
    ...
    2. Copy TLS Secret to APISIX Namespace
        ...
        The TLS secret is required to enable HTTPS for the API Gateway. If you are deploying APISIX in a dedicated namespace, copy the `cnx-tls-secret` from the `connections` namespace to the APISIX namespace.

        Execute the following command to copy the TLS secret:

          ```sh
          kubectl get secret cnx-tls-secret -n connections -o yaml | \
            sed 's/namespace: connections/namespace: <<namespace>>/' | \
            kubectl apply -f -
          ```
        ...
        !!! note
            The TLS secret has been renamed from `ingress-nginx-tls-secret` to `cnx-tls-secret` in Connections v8 CR14. If you previously copied `ingress-nginx-tls-secret` to the APISIX namespace, delete it and copy the new `cnx-tls-secret` instead.

        !!! critical
            When you copy the new `cnx-tls-secret` to the APISIX namespace, you **must** also import the new certificate into your IBM HTTP Server (IHS) keystore. **Failure to complete this step will cause TLS communication between IHS and the API Gateway to fail, resulting in production downtime.** Refer to [Import the Certificate into IBM HTTP Server (IHS)](./enable_ingress_tls.md#how-to-enable-tls-for-the-ingress-controller) for detailed instructions.
    ...
    4. Verify TLS Secret for APISIX Gateway
        ...
        - Verify that the TLS secret (`cnx-tls-secret`) copied in step 2 (or existing in the `connections` namespace) is correctly configured and available in the APISIX namespace.

        - Confirm that the secret name (`cnx-tls-secret`) matches the value referenced in your custom values file `core-apisix-custom-values.yaml` used in the [Install APISIX and HCL API Gateway using Helm](#install-apisix-and-hcl-api-gateway-using-helm) section for APISIX and ingress.

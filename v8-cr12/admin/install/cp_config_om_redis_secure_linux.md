# Securing Redis traffic to the Component Pack \(Linux\) {#cp_config_om_redis_secure_linux .task}
...
1.  Create a SSH tunnel service
    ...
    2. Edit `redis-tunnel-websphere.service` and add the following (replace `<HAProxy FQDN`> with the HAProxy host):
...
1.  Create SSH Tunnel Wrapper Script
    ...
    1. Create a script `/usr/local/bin/start-redis-tunnel.sh` and grant it execute permissions. 
...
2.  Create a SSH tunnel service template
    ...
    2. Edit `redis-tunnel-k8s@.service` and add the following content:
...
Use the `configureRedis.sh` script described in [Running the configuration script](cp_config_om_redis_enable.html#running-the-configuration-script) to set the Redis host (such as,`-m` parameter) to be `localhost`.
...
Follow [Verifying Redis server traffic](cp_config_om_redis_verify.md) to validate that traffic is flowing through the SSL tunnels properly. 

...
The following roadmap is a guide to upgrading to HCL Connections 7.0 by installing it in a new environment and then migrating data from your existing environment. In this example, the existing environment is Connections 6.5 CR1.

The whitepaper that is cited in these steps can be found here: [Upgrading from Connections 6.5 to 7: Side by Side](https://support.hcltechsw.com/csm?id=kb_article&sysparm_article=KB0093267).

This approach can also be applied to upgrading to Connections 7 from versions earlier than 6.5, in which case additional database schema updates would be required.

## Pre-installation tasks {#section_nwy_yqj_kqb .section}

These are some key tasks before installing version 7.0 of Connections in the new environment. For more detail on these tasks, plus some that are optional, see the whitepaper.
...
1.  Prepare to install software:
    -   Review the [software and hardware requirements](https://support.hcltechsw.com/csm?id=kb_article&sysparm_article=KB0073654) for the system that will host Connections 7.
...
3.  If you need to populate the Profiles database with people from your company's LDAP, you will use the Connections 7 wizards package and merge its default assembly line with your existing Connections 6.5 CR1 configuration. In this case take care to follow the steps in the *Populate the Profiles database* section of the whitepaper.
...
## Installing Connections 7 {#section_fpl_crj_kqb .section}

See [Installing HCL Connections 7.0](../install/t_install_cluster.md), taking care to read the suggestions under *About this task*.
...
3.  If you installed the Feature Foundation module during the Connections 7 installation, see the post-installation task [Post-installation tasks for PDF Export](../install/install-guide-preparations.md).

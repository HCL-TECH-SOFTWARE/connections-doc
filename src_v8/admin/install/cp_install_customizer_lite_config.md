# Configuring Customizer Lite {#cp_install_customizer_lite_config .concept}

Configuring HCL Connections for Customizer Lite.

## Configuring Connections: { .section}

Log into the Connections machine and configure the following aspects.

## Modifying HTTPServer { .section}
...
## Modifying Lotus-Connections config { .section}
...
3.  Add the `isExternal` attribute to the Sametime configuration in LotusConnections-config.xml if you configured Sametime awareness through the Sametime server as described in [Adding Sametime awareness through the Sametime server](../admin/t_admin_common_add_st_awareness_via_proxy.md).
4.  Sync the nodes and restart Connections.

## Using Customizer {#proof_of_concept .section}

Refer to [Injecting customizations into Connections pages](../customize/customize_inject_customizations.md) for additional information.

## Useful commands {#ha_install .section}
...
## Changing maximum allowed HTTP header size {#section_gjy_hkn_m5b .section}
